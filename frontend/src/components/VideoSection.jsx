...
export default function VideosSection() {
    ...
    return (
        <section className="bg-gray-100 max-w-7xl mx-auto px-4 py-12">
            <div className="flex justify-between items-center mb-10 flex-wrap">
                ...
                <div className="md:hidden w-full">
                    <select
                        ...
                        className="w-full py-2 px-4 rounded-full font-semibold text-gray-700 bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#DD242D] focus:border-transparent appearance-none"
                    >
                        {filters.map(filter => (
                            <option key={filter} value={filter} className={activeFilter === filter ? 'bg-[#DD242D] text-white' : ''}>
                                {filter}
                            </option>
                        ))}
                    </select>
            ...
            <div className="overflow-x-auto md:overflow-hidden md:grid md:grid-cols-4 gap-6 scrollbar-hide">
                {videos.map(video => (
                    <a
                        key={video.id}
                        ...
                        className="bg-white rounded-xl overflow-hidden shadow-lg border border-gray-200 block transform hover:scale-105 transition-transform duration-300 snap-center w-64 flex-shrink-0 md:w-auto"
                    >
